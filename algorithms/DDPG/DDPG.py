...
class DDPG(AlgorithmAbstract):
    def __init__(self, env_dir: str, kernel_size: int, kernel_dim: int, act_dim: int, buf_size: int,
                 ...
                 pi_lr: float = hyperparams['pi_lr'], q_lr: float = hyperparams['q_lr'],
                 train_iters: int = hyperparams['train_iters'], target_update_freq: int = hyperparams['target_update_freq']):
        super().__init__()
        ...
        self._q_optimizer = Adam(self._model.q_critic.parameters(), lr=q_lr)
    ...
    def compute_loss_q(self, data):
        ...
        with torch.no_grad():
            ...
            pi_q_target = self._target_model.q_critic.forward(next_obs, mask, pi_target)
            targets = rew + self._gamma * pi_q_target
    ...
    def compute_loss_pi(self, data):
        ...
        pi_q_vals = self._model.q_critic.forward(obs, mask, pi).mean()
