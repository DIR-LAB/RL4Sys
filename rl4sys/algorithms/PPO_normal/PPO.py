...
class PPO():
    ...
    def __init__(self, 
        ...
        # This enables accelerated training on machines equipped with CUDA-capable GPUs.
    ...
    def receive_trajectory(self, trajectory: RL4SysTrajectory, version: int) -> bool:
        ...
        for i, r4a in enumerate(trajectory):
            ...
            # Ensure observation tensor resides on the same device as the model
            obs_tensor = torch.as_tensor(r4a.obs, dtype=torch.float32).to(self.device)
            obs_value = self._model_train.get_value(obs_tensor)
            # Store transition)
        ...
        if self.traj > 0 and self.traj > self._traj_per_epoch: 
            ...
            #check model
            #print(f"model pi parameters: {self._model_train.pi.parameters()}")
            #exit()
