...
class CustomCSVLogger:
    ...
    def log_episode(
        ...
        self.writer.add_scalar("sjf_score", sjf_score, self.iteration)
        self.writer.add_scalar("f1_score", f1_score, self.iteration)
        self.writer.add_scalar("episode_return", episode_return, self.iteration)
...
class ActorCritic(nn.Module):
    ...
    ----------
    obs_dim : int
        Dimension of flattened observation (MAX_QUEUE_SIZE * JOB_FEATURES)
    act_dim : int
        Number of discrete actions available.
    attn : bool, optional
        Whether to use the (not-yet-ported) attention policy head. Defaults to ``False``.
    value_variant : str, optional
        One of ``{"v1", "v2", "v3"}`` selecting the value-network depth / width.
        Defaults to ``"v1"`` (three hidden layers of 128 units).
    """
    ...
    def __init__(
        ...
        value_variant = value_variant.lower()

        # ---------------- Policy head variants ----------------
        self.policy_variant = "v1"  # hard-code to v1 as requested

        if self.policy_variant == "kernel":
            # original per-job kernel
        ...
        elif self.policy_variant == "v3":
            layers = []
            in_dim = obs_dim
            for _ in range(5):
                layers += [nn.Linear(in_dim, 32), nn.ReLU()]
                in_dim = 32
            layers.append(nn.Linear(32, act_dim))
            self.policy_net = nn.Sequential(*layers)
        else:
            raise ValueError("Unknown policy variant")

        # ---------------- Attention head (used when self.attn is True) --------------
        if self.attn:
        ...
        if value_variant == "v1":
            ...
            raise ValueError(
                "Unknown value_variant '{}'. Choose from 'v1', 'v2', 'v3'.".format(value_variant)
            )
    ...
    def _rl_kernel(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        logits = self.pol_fc_logits(x).squeeze(-1)
        return logits

    def _policy_logits(self, obs: torch.Tensor) -> torch.Tensor:
        """Return raw action logits of shape (batch, act_dim)."""
        if self.attn:
            batch = obs.shape[0]
            obs_reshaped = obs.view(batch, MAX_QUEUE_SIZE, JOB_FEATURES)
            return self._attention_logits(obs_reshaped)
        elif self.policy_variant == "kernel":
            batch = obs.shape[0]
            obs_reshaped = obs.view(batch, MAX_QUEUE_SIZE, JOB_FEATURES)
            return self._rl_kernel(obs_reshaped)
        else:
            # MLPs operate on flattened observation
