...
class DummyLogger:
    """Mimics the subset of spinup EpochLogger used here."""
    ...
    # tabular logging ----------------------------------------------------------
    def log_tabular(self, *_args, **_kwargs):
        pass
...
FILE_NAME= "SDSC-SP2-1998-4.2-cln"
DATA_FILE= "/home/yomi/0Projects/RL4Sys/rl4sys/examples/job_schedual_old/HPCSim/data/SDSC-SP2-1998-4.2-cln.swf"
MODEL_FILE= "/home/yomi/0Projects/RL4Sys/rl4sys/examples/job_schedual_old/HPCSim/data/SDSC-SP2-1998-4.2-cln.schd"
# -----------------------------------------------------------------------------
...
def ppo(
    ...
    # Replace spinup logger with DummyLogger
    logger = DummyLogger("./logs")
    ...
    ac = ActorCritic(obs_dim, act_dim, attn=attn).to(device)
    ...
    def update():
        ...
        act_t = torch.as_tensor(act_buf, dtype=torch.long, device=device).squeeze(-1)
        mask_t = torch.as_tensor(mask_buf, dtype=torch.float32, device=device)
        ...
        logp_old_t = torch.as_tensor(logp_old_buf, dtype=torch.float32, device=device)
    ...
    for epoch in range(epochs):
        ...
        while True:
            ...
            #print("observation: ", o.tolist())

            obs_torch = torch.as_tensor(o, dtype=torch.float32, device=device).unsqueeze(0)
            mask_torch = torch.as_tensor(mask_np, dtype=torch.float32, device=device).unsqueeze(0)
        ...
        print(f"Epoch {epoch} finished. Time elapsed: {time.time() - start_time:.2f}s")
