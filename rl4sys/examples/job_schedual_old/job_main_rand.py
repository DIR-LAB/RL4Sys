...
import csv
import datetime
# Random baseline: no RL4Sys agent required
...
class JobSchedulingSim:
    ...
    def __init__(self, seed: int, client_id: str, performance_metric: int = 0, workload_file: str = '') -> None:
        ...
        # Random baseline – no RL agent; actions will be sampled uniformly
        # from valid indices determined by the observation mask.
        ...
        for iteration in range(num_iterations):
            ...
            while episode_count < traj_per_epoch:
                ...
                while not done:
                    ...
                    # --------------------------------------------------
                    # Random action sampling based on *mask* (1 = valid)
                    # --------------------------------------------------
                    t_inf0 = time.perf_counter()
                    valid_idx = np.flatnonzero(action_mask)
                    if valid_idx.size == 0:
                        action = 0  # fallback if no valid action exists
                    else:
