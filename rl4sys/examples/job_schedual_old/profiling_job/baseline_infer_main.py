...
from rl4sys.algorithms.PPO.kernel import RLActor
...
class KernelPolicy(nn.Module):
    """Neural network with *kernel* architecture for action inference only."""

    def __init__(self, obs_dim: int, act_dim: int) -> None:
        super().__init__()
        # Instantiate RLActor with the specialised architecture.
        self.actor: RLActor = RLActor(obs_dim, act_dim, actor_type="kernel")
        self.eval()

    @torch.no_grad()
    def act(self, obs: torch.Tensor, mask: torch.Tensor | None = None) -> int:  # noqa: D401
        """Return an action sampled from the categorical policy distribution."""
        dist = self.actor._distribution(  # pylint:disable=protected-access
            obs.unsqueeze(0),  # (1, obs_dim)
...
def run_baseline(env = None, num_steps: int = 4000, workload_file: str | None = None) -> None:
    ...
    # ---------------------------------------------------------------------
    policy = KernelPolicy(OBS_DIM, ACT_DIM)
    ...
    for _ in range(num_steps):
        ...
        # ------------------------ Inference timing -----------------------
        t0 = time.perf_counter()
        action = policy.act(obs_tensor, mask_tensor)
