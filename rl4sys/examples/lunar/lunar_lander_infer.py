...
class LunarLanderSim():
    ...
    def run_application(self, num_iterations: int, max_moves: int) -> None:
        """Run multiple iterations of the Lunar Lander simulation in inference-only mode.

        The agent produces actions using its local policy network, but **does not**
        add any information to a trajectory buffer or communicate with the training
        server. All calls related to trajectory creation, buffering, or reward
        updates have been commented-out.
        ...
        # -----------------------------------------------------------------
        env_time_acc: float = 0.0  # Seconds spent in env.step()
        infer_time_acc: float = 0.0  # Seconds spent generating actions
        ...
        for iteration in range(num_iterations):
            ...
            while not done or moves < max_moves:
                ...
                # Get action from actor ONLY (trajectory handling disabled)
                ...
                # record reward (disabled in inference-only mode)
                ...
                if moves >= max_moves or done:
                    ...
                    # If step exceeds MOVE_SEQUENCE_SIZE or done, set done to True (disabled)
    ...
    def build_observation(self, obs: np.ndarray) -> torch.Tensor:
        """Convert an environment observation to a batch-dimensioned tensor."""
