...
class LunarLanderSim():
    def __init__(self, seed, client_id, performance_metric=0, render_game=False, debug=False):
        ...
        # self.rlagent = RL4SysAgent(conf_path='./rl4sys/examples/lunar/luna_conf.json')  # Disabled: random policy benchmark
    ...
    def run_application(self, num_iterations: int, max_moves: int) -> None:
        """Run several Lunar Lander episodes using purely random actions.

        No policy network is called and no trajectories are recorded; each step
        samples an action directly from the environment's action space.

        Args:
            num_iterations: Number of episodes to simulate.
            max_moves: Maximum number of steps allowed per episode.
        """
        self.logger.info(
            "Starting simulation",
        ...
        # -----------------------------------------------------------------
        env_time_acc: float = 0.0  # Seconds in environment
        infer_time_acc: float = 0.0  # Seconds producing random action (very small)
        ...
        for iteration in range(num_iterations):
            ...
            while not done or moves < max_moves:
                ...
                # Sample a purely random action (agent disabled)
                ...
                # self.rl4sys_action.update_reward(reward)  # Disabled: random policy benchmark
                ...
                if moves >= max_moves or done:
                    ...
                    # self.rlagent.mark_end_of_trajectory(self.rl4sys_traj, self.rl4sys_action)  # Disabled: random policy benchmark
    ...
    def build_observation(self, obs: np.ndarray) -> torch.Tensor:
        """Convert environment observation to tensor (unchanged)."""
