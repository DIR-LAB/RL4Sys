...
def build_observation(cur_snap: "Client_Snapshot") -> torch.Tensor:
    """Construct a feature tensor for the RL model using the first OSC's derived features.

    For simplicity we use the 12 handcrafted features defined in
    `feature_name_list` (see lines ~70). The features are taken from the first
    OSC of the snapshot and returned as a 1×N tensor suitable for the
    RL4SysAgent.
    """
    # Pick the first OSC deterministically so the observation dimensionality is fixed
    osc_name: str = cur_snap.osc_names[0]
    osc_snap = cur_snap.osc_snapshots[osc_name]
    features: List[str] = osc_snap.construct_features_list()
    # Convert all elements to float
    feat_floats: List[float] = [float(v) for v in features]
    return torch.tensor(feat_floats, dtype=torch.float32).unsqueeze(0)

def compute_reward(cur_snap: "Client_Snapshot") -> float:
    """Compute reward as the negative average RPC wait-time.

    For each OSC we take `osc_req_waittime_data.get_average()` (µs) and
    average across all OSCs.  A **lower** wait-time is better, so we return its
    negative value.  If there is no sample data yet, reward is 0.
    """
    total_wait: float = 0.0
    valid_cnt: int = 0
    for osc_name in cur_snap.osc_names:
        osc = cur_snap.osc_snapshots[osc_name]
        avg_wait = osc.osc_req_waittime_data.get_average()  # µs
        if avg_wait > 0:
            total_wait += avg_wait
            valid_cnt += 1
    ...
    if valid_cnt == 0:
        return 0.0
    return -(total_wait / valid_cnt)  # negative: lower wait-time ⇒ higher reward

def apply_action_to_system(cur_snap: "Client_Snapshot", action_id: int) -> None:
    """Map RL action (0/1/2) to parameter adjustments for every OSC."""
    if action_id == 0:
        mppr_choice = "unchanged"
        mrif_choice = "unchanged"
    elif action_id == 1:
        mppr_choice = "double"
        mrif_choice = "double"
    else:
    ...
    for osc_name in cur_snap.osc_names:
        osc_snap = cur_snap.osc_snapshots[osc_name]
        apply_mppr_action(osc_snap, mppr_choice)
        apply_mrif_action(osc_snap, mrif_choice)

def run_rl_simulation(result_folder_path: str, wld_name: str, max_steps: int = 30000) -> None:
    """Run the Lustre RPC tuning loop controlled by an RL4Sys agent."""
    agent_conf_path: str = os.path.join(os.path.dirname(__file__), 'lustre_conf.json')
    rl_agent: RL4SysAgent = RL4SysAgent(conf_path=agent_conf_path)
    rl_traj = None
    rl_action_obj = None

    # TensorBoard writer
    log_dir = os.path.join('./lustre_log', datetime.datetime.now().strftime('%Y%m%d-%H%M%S'))
    ...
    for step in range(max_steps):
        # Populate snapshot & derive features
        cur_snap.populate_snapshot()
        record_dur = round((time.time() - begin_time), 4)
        save_features(cur_snap, prev_snap, record_dur)

        # Build observation tensor
        obs_tensor: torch.Tensor = build_observation(cur_snap)

        # Request action from agent
        rl_traj, rl_action_obj = rl_agent.request_for_action(rl_traj, obs_tensor)
        rl_agent.add_to_trajectory(rl_traj, rl_action_obj)
        # cehck rl_traj size
        print("----> rl_agent._trajectory_buffer size: ", len(rl_agent._trajectory_buffer))
        # print(rl_action_obj.act)

        action_id: int = int(rl_action_obj.act.item()) if hasattr(rl_action_obj.act, 'item') else int(rl_action_obj.act)
        # Map and apply action to system
        apply_action_to_system(cur_snap, action_id)

        # Compute reward & update action
        reward_val: float = compute_reward(cur_snap)
        rl_agent.update_action_reward(rl_action_obj, reward_val)
        # TensorBoard logging
        tb_writer.add_scalar('reward', reward_val, global_step=step)

        # Mark trajectory step completion periodically
        if (step + 1) % 20 == 0:
            rl_action_obj.done = True
            rl_agent.mark_end_of_trajectory(rl_traj, rl_action_obj)
            # rl_traj = None  # Start a new trajectory next step

        # Persist stats for offline analysis
        cur_snap.store_params_to_csv(result_folder_path, wld_name, record_dur)
    ...
    # Cleanly close agent resources
    rl_agent.close()
    tb_writer.close()

# ===================== MAIN ENTRYPOINT =====================
# python rl4sys/examples/lustre_rpc/tuning_rpc_param_rl.py ./rpc_log rl_lustre 0
if __name__ == "__main__":
    result_folder_path = sys.argv[1]
    wld_name = sys.argv[2]
    mode_flag = int(sys.argv[3])  # 0 => RL mode, 1 => default data collection

    if not os.path.exists(result_folder_path):
            os.makedirs(result_folder_path, exist_ok=True)

    if mode_flag == 0:
        run_rl_simulation(result_folder_path, wld_name)
    else:
        # Fall back to original behaviour (data collection only)
        if is_dflt == 1:
            is_dflt_txt = "dflt"
        ...
        apply_action_to_system(cur_snap, 1)
        apply_action_to_system(cur_snap, 1)
        # 1. Populate snapshot & derive features
        begin_time = time.time()
        cur_snap.populate_snapshot()
        ...
        while True:
            ...
            reward_val = compute_reward(cur_snap)
            tb_writer.add_scalar('reward', reward_val, global_step=step)
            step += 1
